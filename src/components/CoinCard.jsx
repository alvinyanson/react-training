...
function CoinCard({ currency, coin }) {
    return (
        <>
            <div class="card bg-dark text-light mb-4">
                <img src={coin.image} class="card-img-top p-5" alt="coin image" />
                <div class="card-body">
                    {/* symbol */}
                    ...
                    <div className='d-flex justify-content-between'>
                        {/* currency symbol */}
                        <span>{getCurrencySymbol(currency)}
                            {formatNumberWithCommas(coin.current_price)}
                        </span>

                        {/* Price change percentage 24 hrs */}
                        <span className={`badge text-bg-light ${coin.price_change_percentage_24h > 0 ? 'text-success' : 'text-danger'}`}>
